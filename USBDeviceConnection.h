...
public:
  ...
  bool sendMidiMessage(const uint8_t* data, size_t length) override {
    ...
    switch (status) {
      case 0x90:
        if (length >= 3) {
          _midi.noteOn(data[1], data[2], channel);
          return true;
        }
        break;
      case 0x80:
        if (length >= 3) {
          _midi.noteOff(data[1], data[2], channel);
          return true;
        }
        break;
      case 0xB0:
        if (length >= 3) {
          _midi.controlChange(data[1], data[2], channel);
          return true;
        }
        break;
      case 0xC0:
        if (length >= 2) {
          _midi.programChange(data[1], channel);
          return true;
        }
        break;
      case 0xD0:
        if (length >= 2) {
          _midi.channelPressure(data[1], channel);
          return true;
        }
        break;
      case 0xE0:
